import React from "react";
import { useNavigate } from "react-router-dom";

function Nav({ cartItem, setSelectedCategory }) {
  const navigate = useNavigate();
  return (
    <>
      <div className="navbar bg-primary text-primary-content">
        <div className="navbar-start">
          <div className="dropdown" >
            <label tabIndex={0} className="btn btn-ghost btn-circle">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-5 w-5"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth="2"
                  d="M4 6h16M4 12h16M4 18h7"
                />
              </svg>
            </label>
            <ul
              tabIndex={0}
              className="menu menu-sm dropdown-content mt-3 p-2 shadow bg-base-100 rounded-box w-52 z-10"
            >
              <li>
                <a onClick={() => navigate("/")}>Homepage</a>
              </li>
              <li>
                <a
                onClick={() => {
                  setSelectedCategory("Baby Toys");
                  navigate("/productList");
                }}>Baby Toys</a>
              </li>
              <li>
                <a 
                onClick={() => {
                  setSelectedCategory("Stroller");
                  navigate("/productList");
                }}>Stroller</a>
              </li>
              <li>
                <a 
                 onClick={() => {
                  setSelectedCategory("Baby Gifts");
                  navigate("/productList");
                }}>Baby Gifts</a>
              </li>
              <li>
                <a 
                onClick={() => {
                  setSelectedCategory("Furniture");
                  navigate("/productList");
                }}
                >Furniture</a>
              </li>
              <li>
                <a>About</a>
              </li>
            </ul>
          </div>
        </div>
        <div className="navbar-center">
          {/* <a className="btn btn-ghost normal-case text-xl">newbie</a> */}
          <div className="logo"></div>
        </div>

        <div className="navbar-end">
          <button
            className="btn btn-ghost btn-circle"
            onClick={() => navigate("/adminLoginPage")}
          >
            <ion-icon name="person-outline"></ion-icon>
          </button>
          <div className="flex-none">
            <div className="dropdown dropdown-end">
              <label tabIndex="0" className="btn btn-ghost btn-circle">
                <div className="indicator">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="h-5 w-5"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      strokeWidth="2"
                      d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"
                    />
                  </svg>
                  <span className="badge badge-sm indicator-item">
                    {cartItem &&
                      cartItem.reduce((acc, cur) => {
                        return acc + cur.quantity;
                      }, 0)}
                  </span>
                </div>
              </label>
              <div
                tabIndex="0"
                className="mt-3 card card-compact dropdown-content w-52 bg-base-100 shadow z-10"
              >
                <div className="card-body">
                  <span className="font-bold text-lg">
                    {cartItem &&
                      cartItem.reduce((acc, cur) => {
                        return acc + cur.quantity;
                      }, 0)}{" "}
                    Items
                  </span>
                  <span className="text-info">
                    Subtotal: €
                    {cartItem &&
                      cartItem.reduce((acc, cur) => {
                        return acc + Number(cur.quantity) * Number(cur.price);
                      }, 0)}
                  </span>
                  <div className="card-actions">
                    <button className="btn btn-primary btn-block">
                      View cart
                    </button>
                  </div>
                </div>
              </div>
            </div>
          </div>

          <button className="btn btn-ghost btn-circle">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-5 w-5"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth="2"
                d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"
              />
            </svg>
          </button>
        </div>
      </div>

      <div className="navbar-center hidden lg:flex bg-primary lg:justify-center">
        <ul className="menu menu-horizontal px-1 uppercase">
          <li>
            <a onClick={() => navigate("/")}>Home</a>
          </li>
          <li>
            <a
              onClick={() => {
                setSelectedCategory("Baby Toys");
                navigate("/productList");
              }}
            >
              Baby Toys
            </a>
          </li>
          <li tabIndex={0}>
            <details>
              <summary>Stroller</summary>
              <ul className="p-2">
                <li>
                  <a
                  onClick={() => {
                    setSelectedCategory("Stroller");
                    navigate("/productList");
                  }}
                  
                  >
                    Buggies</a>
                </li>
                <li>
                  <a 
                  onClick={() => {
                    setSelectedCategory("Stroller");
                    navigate("/productList");
                  }}
                  >3-in-1 Stroller</a>
                </li>
              </ul>
            </details>
          </li>
          <li>
            <a 
            
            onClick={() => {
              setSelectedCategory("Baby Gifts");
              navigate("/productList");
            }}>Baby Gifts</a>
          </li>
          <li>
            <a
            onClick={() => {
              setSelectedCategory("Furniture");
              navigate("/productList");
            }}
            >Furniture</a>
          </li>
        </ul>
      </div>
    </>
  );
}

export default Nav;
